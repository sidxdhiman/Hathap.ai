...
import { RedTeamSeverity, RedTeamFindingType } from './types';
 ...
 * - Claims that have contradicting evidence
  ...
  async runRedTeamAnalysis(params: {
    ...
    const claims = claimIds.length > 0
      ? await Claim.find({ _id: { $in: claimIds }, decisionId })
      : [];
    const evidenceDocs = evidenceIds.length > 0
      ? await Evidence.find({ _id: { $in: evidenceIds }, decisionId })
      : [];

    const findings: IRedTeamFinding[] = [];
    ...
    const allClaimEvidenceIds = new Set<string>();
    for (const claim of claims) {
      for (const eid of claim.evidenceIds || []) {
        allClaimEvidenceIds.add(eid);
      }
    }
    const orphanedEvidence = evidenceDocs.filter(
      (e) => !allClaimEvidenceIds.has(e._id.toString())
    );
   ...
   */
  async getFindingsForDecision(decisionId: string): Promise<IRedTeamFinding[]> {
    return RedTeamFinding.find({ decisionId }).sort({ createdAt: 1 });
  }

  /**
   * Load red-team findings for a specific task.
   */
  async getFindingsForTask(taskId: string): Promise<IRedTeamFinding[]> {
    return RedTeamFinding.find({ taskId }).sort({ createdAt: 1 });
  }

  /**
   * Delete all red-team findings for a decision.
   */
  async deleteFindingsForDecision(decisionId: string): Promise<void> {
    await RedTeamFinding.deleteMany({ decisionId });
  }
