...
const SECRET_KEY_PATTERNS =
  /(api[_-]?key|apikey|secret|token|credential|authorization|auth[_-]?header|password|passwd|private[_-]?key|access[_-]?key|bearer)/i;
...
function redactSecrets(input: unknown, depth = 0): unknown {
  ...
  if (typeof input === 'string') {
    if (input.length > 100000) return '[truncated]';
    let out = input;
...
export class DecisionReportService {
  async generate(userId: string, decisionId: string): Promise<GenerateReportResult> {
    ...
    const [
      ...
      Execution.find({ decisionId }).sort({ createdAt: 1 }),
      Task.find({
        executionId: { $in: (await Execution.find({ decisionId })).map((e) => e._id) },
      }).sort({ priority: 1, createdAt: 1 }),
      Claim.find({ decisionId }).sort({ createdAt: 1 }),
      Evidence.find({ decisionId }).sort({ createdAt: 1 }),
      VerificationResult.find({ decisionId }).sort({ createdAt: 1 }),
      RedTeamFinding.find({ decisionId }).sort({ createdAt: 1 }),
      ReconciliationResult.findOne({ decisionId }),
      ExecutionEvent.find({ decisionId }).sort({ createdAt: 1 }),
      EvaluationCaseResult.find({ 'artifact.decisionId': decisionIdStr }).sort({ createdAt: 1 }),
    ]);
    ...
    md.push('');
    md.push(
      `Generated ${new Date().toISOString()} · Decision \`${decisionIdStr}\` · Status **${decision.status}**`
    );
    md.push('');
    ...
    if (recommendation && recommendation.trim()) {
      ...
      md.push(
        '_No recommendation has been produced yet. The decision has not been reconciled, so any verdict below would be unverified._'
      );
    }
    ...
    if (reconciliation?.rationale && reconciliation.rationale.trim()) {
      ...
      if (verdictEvidence?.content) md.push(truncate(verdictEvidence.content, 4000));
      else md.push('_No executive summary is available until the decision is reconciled._');
    }
    ...
    if (evidenceList.length === 0) {
      ...
      evidenceList.slice(0, 50).forEach((e, i) => {
        ...
        if (e.provider) md.push(`- **Provider:** ${e.provider === 'mock' ? 'mock (synthetic — not real web research)' : e.provider}`);
        if (e.retrievedAt) md.push(`- **Retrieved:** ${new Date(e.retrievedAt).toISOString()}`);
        if (e.sourceReliability) md.push(`- **Reliability:** ${e.sourceReliability}`);
    ...
    if (claims.length === 0) {
      ...
      claims.slice(0, 100).forEach((c, i) => {
        md.push(
          `${i + 1}. **${c.status}** (${c.type})${typeof c.confidence === 'number' ? `, confidence ${Math.round(c.confidence * 100)}%` : ''}: ${truncate(c.text, 400)}`
        );
      });
    ...
    if (verifications.length === 0) {
      ...
      verifications.slice(0, 100).forEach((v, i) => {
        md.push(
          `${i + 1}. **${v.status}** (mode: ${v.mode}) — ${truncate(v.claimStatement, 300)}`
        );
        if (v.rationale) md.push(`   - ${truncate(v.rationale, 400)}`);
      });
    }
    ...
    if (findings.length === 0) {
      ...
      findings.slice(0, 100).forEach((f, i) => {
        md.push(
          `${i + 1}. **${f.severity}** / ${f.type} — ${truncate(f.description, 400)}`
        );
        if (f.suggestedAction) md.push(`   - Suggested action: ${truncate(f.suggestedAction, 300)}`);
      });
    }
    ...
    if (!reconciliation) {
      ...
      md.push(
        `- **Surviving claims:** ${reconciliation.survivingClaimIds.length}\n` +
    ...
    md.push('');
    const routingModels = new Set<string>();
    for (const t of tasks) {
      const selection = (t.metadata as any)?.routing?.selection;
      const modelName = selection?.model?.modelName || t.assignedModel;
      const agentName = (t.metadata as any)?.routing?.selection?.agent?.name || t.assignedAgent;
      if (modelName) {
        routingModels.add(
          `${modelName}${selection?.model?.provider ? ` (${selection.model.provider})` : ''}`
        );
      }
      if (agentName) routingModels.add(`agent: ${agentName}`);
    }
    if (routingModels.size === 0) {
      md.push('_No model/agent routing selections were recorded._');
    } else {
      for (const r of Array.from(routingModels).sort()) md.push(`- ${r}`);
    }
    const manualModelId = (executions[0]?.metadata as any)?.routing?.modelId;
    if (manualModelId) md.push(`\nRouting mode: manual (requested model \`${manualModelId}\`)`);
    ...
    if (executions.length === 0) {
      ...
      for (const e of executions) {
        const usage = (e.tokenUsage as any) || {};
      ...
      for (const e of executions.slice(0, 10)) {
        ...
        md.push(
          `- Execution \`${e._id}\` (${e.status}): ${Math.round(e.progress ?? 0)}% · totalTokens ${
            usage.totalTokens ?? 0
          }${modelLine ? ` · ${modelLine}` : ''}`
        );
      }
    ...
    if (evalResults.length === 0) {
      md.push(
        '_No automated evaluation results reference this decision. Run an evaluation benchmark (decision-engine mode) that uses this artifact to populate this section._'
      );
    } else {
      for (const r of evalResults.slice(0, 10)) {
        const score = (r.metrics as any)?.score;
        md.push(
          `- **${r.caseTitle || r.caseId}** — status ${r.status}${typeof score === 'number' ? `, composite score ${(score * 100).toFixed(1)}%` : ''} (run \`${r.runId}\`)`
        );
      }
    }
    ...
    if (decision.completedAt) md.push(`- **Completed:** ${decision.completedAt.toISOString()}`);
    for (const e of executions) {
      md.push(
        `- **Execution** \`${e._id}\` — ${e.status}${e.startedAt ? `, started ${e.startedAt.toISOString()}` : ''}${e.completedAt ? `, completed ${e.completedAt.toISOString()}` : ''}`
      );
    }
...
export const decisionReportService = new DecisionReportService();