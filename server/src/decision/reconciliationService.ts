...
export class ReconciliationService {
  ...
  async runReconciliation(params: {
    ...
    const claimVerificationMap = new Map<string, string>();
    ...
    const redTeamFindings = redTeamTaskId
      ? await RedTeamFinding.find({ decisionId, taskId: redTeamTaskId })
      : [];

    const highCriticalFindings = redTeamFindings.filter(
      (f) => f.severity === 'high' || f.severity === 'critical'
    );

    // Categorize claims
    const survivingClaimIds: string[] = [];
    ...
    for (const vr of verificationResults) {
      if (
        vr.supportingEvidenceIds.length > 0 &&
    ...
    if (rejectedClaimIds.length > 0) {
      ...
      researchQuestions.push(
        `Re-evaluate the recommendation in light of ${rejectedClaimIds.length} contradicted claim(s).`
      );
    }
    ...
    // Persist
    const result = await ReconciliationResult.findOneAndUpdate(
      { decisionId, taskId },
      {
        decisionId,
        executionId,
   ...
   */
  async getReconciliationForDecision(decisionId: string): Promise<IReconciliationResult | null> {
    return ReconciliationResult.findOne({ decisionId });
  }

  /**
   * Delete reconciliation results for a decision.
   */
  async deleteReconciliationForDecision(decisionId: string): Promise<void> {
    await ReconciliationResult.deleteMany({ decisionId });
  }

  private buildReconciliationRationale(params: {
    ...
    if (params.redTeamFindingCount > 0) {
      parts.push(
        `Red team produced ${params.redTeamFindingCount} finding(s) (${params.highCriticalFindingCount} high/critical).`
      );
    }
