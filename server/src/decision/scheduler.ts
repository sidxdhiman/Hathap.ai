...
export interface ScheduleDecision {
  executableTasks: SchedulableTask[];
  blockedTasks: SchedulableTask[];
}
...
export function schedule(
  ...
  const executableTasks: SchedulableTask[] = [];
  const blockedTasks: SchedulableTask[] = [];

  const isActive = executionStatus === 'running' || executionStatus === 'queued';

  const statusById = new Map<string, ITask>();
  for (const t of tasks) statusById.set(t._id.toString(), t);
