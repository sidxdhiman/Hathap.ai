...
export class MajorityVoteStrategy implements DebateStrategy {
  async execute(ctx: DebateContext): Promise<DebateResult> {
    ...
    const round1Promises = ctx.agents.map((agent) =>
      runAgent(
        ...
        [],
        1,
        'Provide your initial analysis, perspectives, and key arguments on the objective.'
      )
    );
    ...
    const round2Promises = ctx.agents.map((agent) =>
      runAgent(
        ...
        messages,
        2,
        'Review the arguments. You must explicitly cast your vote as "APPROVE" or "REJECT" in your position field, and provide your voting justification in the arguments/recommendation.'
      )
    );
