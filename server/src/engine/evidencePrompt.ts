 ...
 */
export function buildEvidenceBlock(evidence: EvidenceView[]): string {
  const views = (evidence || []).slice(0, MAX_EVIDENCE);
  if (views.length === 0) return '';

  const lines = views.map((e, i) => {
    const meta: string[] = [];
    if (e.sourceName) meta.push(`source=${e.sourceName}`);
    if (e.sourceUrl) meta.push(`url=${e.sourceUrl}`);
    if (e.provenanceKind) meta.push(`provenance=${e.provenanceKind}`);
    if (e.sourceReliability) meta.push(`reliability=${e.sourceReliability}`);
