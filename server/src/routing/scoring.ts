...
import { TASK_TYPE_SOFT_REQUIREMENTS } from './routingPolicy';
...
export function softRequirementsForTaskType(taskType: TaskType): string[] {
  return TASK_TYPE_SOFT_REQUIREMENTS[taskType] || [];
}

function modelLookupKey(model: { modelName?: string; displayName?: string }): string {
...
function costFactorFor(
  ...
  return {
    value: 1 - normalized,
    note: `estimated cost ${estimate.estimatedCost.toFixed(6)} within candidate range [${
      min.toFixed(6)}..${max.toFixed(6)}]`,
  };
}
...
export function scoreCandidates(
  ...
  return candidates.map((candidate, idx) => {
    ...
    const quality =
      policy.qualityOverrides[modelName] ?? policy.qualityOverrides[model.modelName || ''] ?? policy.neutralQuality;
    factors.push(
      makeFactor('quality', quality, policy, qualityNote(quality, model.modelName, modelName, policy))
    );
    ...
    const softMatched = input.soft.length
      ...
      : 1;
    factors.push(
      makeFactor('capability', softMatched, policy, capabilityNote(softMatched, input.soft, agentCaps))
    );

    // 3. specialization — Jaccard between agent capabilities and task requirements.
    const spec = jaccard(agentCaps, allReqs);
    factors.push(
      makeFactor('specialization', spec, policy, `capability overlap (${agentCaps.join(', ') || 'none'})`)
    );

    // 4. reliability — model status prior.
    const reliability = policy.reliabilityByStatus[model.status || 'untested'] ?? policy.neutralQuality;
    factors.push(
      makeFactor('reliability', reliability, policy, `model status "${model.status || 'untested'}"`)
    );
    ...
    const latency =
      policy.latencyOverrides[modelName] ?? policy.latencyOverrides[model.modelName || ''] ?? policy.neutralLatency;
    factors.push(
      makeFactor('latency', latency, policy, latencyNote(latency, model.modelName, modelName, policy))
    );
    ...
    const isDiversityTask =
      policy.diversityPreferenceTasks.includes(input.taskType) &&
...
function qualityNote(value: number, bareModelName: string, resolvedKey: string, policy: RoutingPolicy): string {
...
function capabilityNote(value: number, soft: string[], caps: string[]): string {
  if (soft.length === 0) return 'no soft requirements for this task type';
  return value === 1
    ? `agent covers all soft requirements: ${soft.join(', ')}`
    : `agent covers ${value * soft.length}/${soft.length} soft requirements (${soft.join(', ')})`;
}

function latencyNote(value: number, bareModelName: string, resolvedKey: string, policy: RoutingPolicy): string {
...
export function pickBestCandidate(candidates: RoutingCandidate[]): RoutingCandidate | undefined {
...
export function buildRoutingReasons(
  ...
  for (const factor of score.factors) {
    if (factor.name === 'quality' || factor.name === 'latency' || factor.name === 'cost') continue;
    if (factor.note) reasons.push(`${factor.name}: ${factor.note}`);
  }
