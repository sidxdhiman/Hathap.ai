...
export class DuckDuckGoResearchSource implements ResearchSource {
  ...
  async search(query: string, options?: ResearchOptions): Promise<ResearchResult[]> {
    ...
    if (response.status === 401 || response.status === 403) {
      throw new ResearchError('AUTHENTICATION_FAILURE', 'Research provider rejected authentication.');
    }
    if (response.status === 400) {
      throw new ResearchError('INVALID_QUERY', `Research provider rejected the query (HTTP 400).`);
    }
    if (!response.ok) {
      throw new ResearchError('PROVIDER_UNAVAILABLE', `Research provider failed (HTTP ${response.status}).`);
    }
    ...
    try {
      ...
      throw new ResearchError('CONTENT_FETCH_FAILURE', 'Research provider returned unreadable content.');
    }
    ...
    if (abstractText) {
      results.push({
        title: clampContent(payload.Heading || q, 200),
        url: payload.AbstractURL ? `${payload.AbstractURL}` : undefined,
        sourceName: payload.AbstractSource || 'DuckDuckGo Instant Answer',
        snippet: clampContent(abstractText, 500),
        content: clampContent(abstractText, MAX_TEXT),
    ...
    const related: any[] = Array.isArray(payload.RelatedTopics) ? payload.RelatedTopics : [];
    let relatedCount = 0;
    const maxRelated = Math.max((options?.maxResults || 5) - results.length, 0);
    const walk = (node: any) => {
      if (relatedCount >= maxRelated) return;
      if (node.Text) {
        const entry: any = { text: node.Text, url: node.FirstURL, sourceName: node.Source };
        relatedCount += 1;
        results.push({
          title: clampContent(String(node.Text).split(' - ')?.[0] || q, 200),
          url: entry.url ? `${entry.url}` : undefined,
          sourceName: entry.sourceName || 'DuckDuckGo Related Topics',
          snippet: clampContent(String(node.Text), 500),
          content: clampContent(String(node.Text), MAX_TEXT),
          publishedAt: undefined,
          retrievedAt: now,
      ...
      if (Array.isArray(node.Topics)) {
        for (const sub of node.Topics) walk(sub);
      }
    };
    for (const topic of related) walk(topic);
