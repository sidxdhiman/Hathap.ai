 ...
 */
const ESCAPE_MARKERS: Array<{ marker: string; code: any; message: string }> = [
...
constructor(
    private options: { fixtures?: ResearchResult[] } = {}
  ) {} 
  ...
  async search(query: string, options?: ResearchOptions): Promise<ResearchResult[]> {
    ...
    for (const { marker, code, message } of ESCAPE_MARKERS) {
      if (q.includes(marker)) throw new ResearchError(code, message);
    }
