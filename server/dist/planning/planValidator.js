 ...
 */
const SYSTEM_GENERATED_TYPES = ['verify_claim', 'red_team', 'reconciliation'];
...
function validatePlan(proposal, options = {}) {
    ...
    if (typeof termination.requiresVerification !== 'boolean' ||
        typeof termination.requiresRedTeam !== 'boolean' ||
        typeof termination.requiresReconciliation !== 'boolean') {
        errors.push('Plan "termination" must declare requiresVerification, requiresRedTeam and requiresReconciliation booleans.');
    }
    ...
    if (taskCount === 0) {
        ...
        errors.push(`Plan proposes ${taskCount} tasks — exceeds maxTasksPerExecution (${policy.maxTasksPerExecution}).`);
    }
    const ids = new Set();
    const byId = new Map();
    let researchCount = 0;
    ...
    for (let i = 0; i < plan.tasks.length; i++) {
        ...
        if (typeof t.tempId !== 'string' || t.tempId.trim().length === 0) {
            ...
            if (ids.has(t.tempId))
                errors.push(`${label} duplicates tempId "${t.tempId}".`);
            ids.add(t.tempId);
            byId.set(t.tempId, t);
        }
        // type
        if (!(0, planningPolicy_1.isAllowedTaskType)(t.type)) {
            errors.push(`${label} uses unknown task type "${String(t.type)}".`);
        }
        else if (SYSTEM_GENERATED_TYPES.includes(t.type)) {
            errors.push(`${label} (${t.type}) may not be planner-proposed: verification, red team and reconciliation are system-generated from the plan termination flags.`);
        }
        // purpose
        ...
        if (Array.isArray(t.requirements)) {
            for (const req of t.requirements) {
                if (typeof req !== 'string' || !(0, planningPolicy_1.isKnownCapability)(req)) {
        ...
        if (t.type === 'research') {
            researchCount++;
            const maxResults = t.input && typeof t.input.maxResults === 'number'
                ? t.input.maxResults
                : 0;
            researchResults += maxResults;
        }
        if (t.type === 'verify_claim')
            verificationCount++;
    }
    if (researchCount > policy.maxResearchTasks) {
        errors.push(`Plan proposes ${researchCount} research tasks — exceeds maxResearchTasks (${policy.maxResearchTasks}).`);
    }
    if (verificationCount > policy.maxVerificationTasks) {
        errors.push(`Plan proposes ${verificationCount} verify_claim tasks — exceeds maxVerificationTasks (${policy.maxVerificationTasks}).`);
    }
    if (researchResults > policy.maxTotalResearchResults) {
        errors.push(`Plan research maxResults total ${researchResults} exceeds maxTotalResearchResults (${policy.maxTotalResearchResults}).`);
    }
    // Dependency integrity + depth + cycle detection over tempId graph.
    const depthErrors = validateDependencies(plan.tasks || [], byId, policy);
    ...
    const normalized = {
        ...
        source: plan.source === 'fallback' || plan.source === 'baseline' ? plan.source : 'intelligent',
...
function validateTaskInput(type, input, label) {
    ...
    if (type === 'research') {
        ...
        for (const [k, v] of Object.entries(input)) {
            if (k === 'query' && typeof v === 'string' && thisIsDangerousText(v)) {
    ...
    if (type === 'debate') {
        ...
        for (const [k, v] of Object.entries(input)) {
            if (typeof v === 'string' && thisIsDangerousText(v)) {
    ...
    for (const v of Object.values(input)) {
        ...
        for (let i = 0; i < nested.length; i++) {
            const n = nested[i];
            if (SSRF_OR_EXECUTION_KEYS.includes(n.key) ||
                CREDENTIAL_KEYS.includes(n.key) ||
                CALLBACK_KEYS.includes(n.key) ||
                (typeof n.value === 'string' && thisIsDangerousText(n.value))) {
...
function validateDependencies(tasks, byId, policy) {
    const errors = [];
    const byTemp = new Map();
    for (const t of tasks) {
        if (typeof t.tempId === 'string')
            byTemp.set(t.tempId, t);
    }
    for (const t of tasks) {
    ...
    for (const t of tasks) {
        ...
        if (td > policy.maxPlanDepth) {
            errors.push(`Plan chain depth ${td} for tempId "${t.tempId}" exceeds maxPlanDepth (${policy.maxPlanDepth}).`);
        }
    }
    return errors;
}
function thisIsDangerousText(value) {
    if (value.trim().length === 0)
        return false;
    if (DANGEROUS_VALUE_PATTERNS.some((re) => re.test(value)))
        return true;
...
function scanNestedValues(value, key = '') {
    if (Array.isArray(value)) {
        const out = [];
        for (const item of value) {
            for (const n of scanNestedValues(item, key))
                out.push(n);
        }
        return out;
    }
    if (value && typeof value === 'object') {
        ...
        for (const [k, v] of Object.entries(value)) {
            ...
            for (const n of scanNestedValues(v, k))
                out.push(n);
        }
        return out;
    }
