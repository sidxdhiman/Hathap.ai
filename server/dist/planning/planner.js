...
exports.decisionPlanner = exports.DecisionPlanner = exports.PlanningError = void 0;
...
const fallbackPlanner_1 = require("./fallbackPlanner");
const planCompiler_1 = require("./planCompiler");
...
class DecisionPlanner {
    constructor(options = {}) {
        this.policy = (0, planningPolicy_1.makePlanningPolicy)(options.policy);
        this.injectedPlanCall = options.planCall;
    }
    async planExecution(options) {
        const execution = await Execution_1.default.findById(options.executionId);
        if (!execution) {
        ...
        const planCall = options.planCall || this.injectedPlanCall || this.buildRealPlanCall(options.userId);
        ...
        if (options.planningMode === 'intelligent' && canAttemptIntelligent) {
            ...
            while (attempts < maxAttempts && !accepted) {
                attempts++;
                const attempt = await this.runAttempt(context, provenance, planCall, this.policy.planningTimeoutMs);
                if (!attempt.ok) {
                    eventBus_1.executionEventBus.emit({
                ...
                const validation = (0, planValidator_1.validatePlan)(parsed, { policy: this.policy });
        ...
        if (!accepted) {
            fallback = (0, fallbackPlanner_1.buildFallbackPlan)(context, {
                planSource: provenance.plannerModel ? 'fallback' : 'baseline',
                researchQueries: options.researchQueries,
                policy: this.policy,
            });
            const fallbackValidation = (0, planValidator_1.validatePlan)(fallback, { policy: this.policy });
            if (!fallbackValidation.valid || !fallbackValidation.plan) {
                await this.failPlanning(execution, {
                ...
                }, usageRecords, startedAt);
                throw new PlanningError('PLAN_NOT_POSSIBLE', `No valid plan could be produced. ${fallbackValidation.errors.join('; ')}`);
            }
    ...
    async tryResumeExisting(execution) {
        const exec = execution;
        if (exec.planningStatus === 'planned' && exec.planId) {
            const plan = await DecisionPlan_1.default.findOne({ executionId: String(execution._id) });
            if (plan && (plan.status === 'validated' || plan.status === 'compiled')) {
                const compiled = await planCompiler_1.planCompiler.compile({
                    execution,
                    plan: this.persistedToPlan(plan),
                    planningMode: exec.planningMode || 'intelligent',
                    policy: this.policy,
                });
                return {
                    executionId: String(execution._id),
    ...
    async runAttempt(context, provenance, planCall, timeoutMs) {
        let timer;
    ...
    async failPlanning(execution, failure, usageRecords, startedAt) {
    ...
    async recordUsage(execution, records) {
        if (!records.length)
            return;
        const agg = (0, usage_1.aggregateUsage)(records);
        ...
        execution.tokenUsage = execution.tokenUsage || {};
    }
    async buildContext(execution, options) {
        ...
        const Claim = (await Promise.resolve().then(() => __importStar(require('../models/Claim')))).default;
        ...
        const participantCaps = (decision?.participants || [])
            .map((p) => p?.capabilities || p?.capability || [])
            .flat()
            .filter((c) => typeof c === 'string');
        const existingEvidence = evidence.map((e) => ({
            id: e._id.toString(),
            title: e.title,
        ...
        context.historicalMemory = await this.buildHistoricalMemory(options.userId, context.decisionId, decision);
        return context;
    }
    /**
     ...
     */
    async buildHistoricalMemory(userId, decisionId, decision) {
        try {
            const { memoryContextBuilder } = await Promise.resolve().then(() => __importStar(require('../memory/memoryContextBuilder')));
            ...
            return await memoryContextBuilder.buildForPlanning({
                ...
                tags: Array.isArray(metadata.tags) ? metadata.tags.filter((t) => typeof t === 'string') : undefined,
                entities: Array.isArray(metadata.entities) ? metadata.entities.filter((e) => typeof e === 'string') : undefined,
            });
        }
        catch (err) {
    ...
    async buildProvenance(execution, options) {
        // Deterministic planner model selection: first-enabled model.
        let plannerModel;
        if (options.planningMode === 'intelligent') {
            const model = await Model_1.default.find({ userId: options.userId, enabled: true })
                .sort({ createdAt: 1 })
                .limit(1);
     ...
     */
    buildRealPlanCall(userId) {
        return async (input) => {
            const model = await Model_1.default.find({ userId, enabled: true }).sort({ createdAt: 1 }).limit(1);
            const m = model[0];
            if (!m) {
            ...
            try {
                ...
                const text = await (0, llmClient_1.callLLM)(m, [
                ...
                const exec = await Execution_1.default.findById(input.provenance.executionId);
                if (exec)
                    await this.recordUsage(exec, usageRecords);
                return { ok: true, text };
            }
            catch (err) {
...
exports.DecisionPlanner = DecisionPlanner;
exports.decisionPlanner = new DecisionPlanner();
