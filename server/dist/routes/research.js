 ...
 */
const DEMO_OBJECTIVE = 'Should a small software startup adopt GraphQL for its public API, considering team size, ' +
 ...
 */
router.get('/status', authMiddleware_1.requireAuth, async (_req, res) => {
    try {
 ...
 */
router.post('/demo', authMiddleware_1.requireAuth, async (req, res) => {
    try {
        const status = (0, researchConfig_1.describeResearchStatus)();
        if (!status.configured || !status.real) {
            return res.status(400).json({
                ...
                setup: (0, researchConfig_1.researchSetupInstructions)(),
                status,
            });
        }
        ...
        const execution = await orchestrator_1.decisionOrchestrator.startDecision(decision._id.toString(), req.userId, {
