...
exports.routeTaskRouter = exports.RouteTaskRouter = void 0;
const Task_1 = __importDefault(require("../models/Task"));
const candidateResolver_1 = require("./candidateResolver");
 ...
 */
class RouteTaskRouter {
    constructor(policy) {
        this.policy = policy || (0, routingPolicy_1.loadRoutingPolicy)();
    }
    static buildForTest(overrides) {
        return new RouteTaskRouter((0, routingPolicy_1.makeRoutingPolicy)(overrides));
    }
    policyVersion() {
    ...
    async routeTask(input) {
        ...
        if (input.routingMode === 'manual') {
            ...
            if (manualCandidates.length === 0) {
                return fail('MANUAL_MODEL_UNAVAILABLE', 'The manually selected model is not available (disabled, failed its connectivity test, or missing an API key).');
            }
        ...
        if (this.policy.maxEstimatedCostPerTask > 0) {
            ...
            if (withinBudget.length === 0) {
                ...
                return fail('NO_CANDIDATE_WITHIN_BUDGET', `No candidate is within the estimated-cost budget ($${maxCost} per task).`);
            }
            pool = withinBudget;
        }
        ...
        if (eligible.length === 0) {
            if (this.policy.allowCapabilityGateRelaxation) {
                ...
                return fail('NO_AGENT_COVERS_REQUIREMENTS', `No agent covers the required capabilities: ${requirements.join(', ')}.`, [requirements.join(', ')]);
            }
        }
        const soft = (0, scoring_1.softRequirementsForTaskType)(taskType);
        const scored = (0, scoring_1.scoreCandidates)(eligible, {
            taskType,
            hard: requirements,
            soft,
            primaryProvider: input.primaryProvider,
        }, this.policy);
        const best = (0, scoring_1.pickBestCandidate)(scored);
        if (!best) {
     ...
     */
    async routeFallbackForRetry(input) {
    ...
    async resolvePrimaryProvider(executionId) {
        const debateTask = await Task_1.default.findOne({ executionId, type: 'debate' }).select('metadata').lean();
        const routing = debateTask?.metadata?.routing;
        return routing?.selection?.model?.provider;
    }
    toSelection(candidate, score, opts) {
        const agent = candidate.agent
            ? {
                id: String(candidate.agent._id),
                name: candidate.agent.name || 'Unnamed agent',
                capabilities: candidate.agent.capabilities || [],
            }
            : null;
        const model = {
        ...
        const reasons = (0, scoring_1.buildRoutingReasons)(candidate, opts.gateRelaxed, opts.primaryProvider);
...
exports.RouteTaskRouter = RouteTaskRouter;
exports.routeTaskRouter = new RouteTaskRouter();
