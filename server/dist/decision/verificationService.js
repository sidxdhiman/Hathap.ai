...
class VerificationService {
    ...
    async verifyClaim(params) {
        ...
        const relationships = await evidenceGraphService_1.evidenceGraphService.getRelationshipsForClaim(claimId);
        ...
        const evidenceDocs = allEvidenceIds.length > 0
            ? await Evidence_1.default.find({ _id: { $in: allEvidenceIds } })
            : [];
        ...
        const result = await VerificationResult_1.default.findOneAndUpdate({ claimId, taskId }, {
            claimId,
            claimStatement,
            status,
     ...
     */
    async getVerificationsForDecision(decisionId) {
        return VerificationResult_1.default.find({ decisionId }).sort({ createdAt: 1 });
    }
    /**
     * Load verification result for a specific claim.
     */
    async getVerificationForClaim(claimId) {
        return VerificationResult_1.default.findOne({ claimId });
    }
    /**
     * Delete all verification results for a decision.
     */
    async deleteVerificationsForDecision(decisionId) {
        await VerificationResult_1.default.deleteMany({ decisionId });
    }
    /**
     ...
     */
    evaluateVerificationStatus(params) {
